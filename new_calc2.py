...
def edit_and_add():
    ...
    st.title("Edit Details & Add New Calculation")
    new_records = []
    ...
    st.write(id_calc_02)
    _id_calc = tmycalc['id_calc'].values[0]; _user_id = tmycalc['user_id'].values[0]
    _well_name = tmycalc['well_name'].values[0]; _field_name = tmycalc['field_name'].values[0]
    _company = tmycalc['company'].values[0]; _engineer = tmycalc['engineer'].values[0]
    _date_calc = tmycalc['date_calc'].values[0]
    
    st.write(_id_calc); st.write(_user_id)
