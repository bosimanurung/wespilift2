...
mycalc3b=sel_row=bs_grid_table=pd.DataFrame()
_well_name_search=''
id_calc_01=0
...
if _well_name_search: #_well_name_search!='':
    ...
    bs_grid_table = AgGrid(mycalc3b, gridOptions=gridoptions,
                            enable_enterprise_modul=True,
                            height=500,
                            allow_unsafe_jscode=True,
                            theme='alpine')
    #st.session_state.sel_row = st.session_state.bs_grid_table["selected_rows"] # DataFrame
    #sel_row = bs_grid_table["selected_rows"] # Is it really a DataFrame
    
    rowIndex = bs_grid_table["grid_response"]
    colIndex = bs_grid_table["grid_response"]
    
    #Selected row - as already discussed
    #selected_rows = bs_grid_table["selected_rows"].iloc[0]
    selected_rows = bs_grid_table["selected_rows"]
    
    #session state keeping the value of the selected cell
    st.session_state.selected_cell = list(bs_grid_table["selected_rows"].iloc[0])[int(col_id)-1]
    
    st.session_state.click = st.button('Add New')
    if st.session_state.click:
        # --------- it's ok:
        #if st.dataframe(sel_row, hide_index=True):
        #    test = sel_row['id_calc'].values[0]
        #    if test:
        #        st.write(test)
        #else:
        #    st.write('empty')
