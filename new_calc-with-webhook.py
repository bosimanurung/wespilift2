...
WEBHOOK_URL = "https://connect.pabbly.com/workflow/sendwebhookdata/IjU3NjYwNTZjMDYzMjA0MzM1MjZhNTUzMTUxM2Ii_pc"
...
# ---------- webhook ----------------------------------------------------------
        if not WEBHOOK_URL:
            st.error('The Webhook service is not set up!')
        else:            
            data={}; f_id_calc='id_calc'
            data = {"idcalc" : new_id_calc, "user_id" : _user_id, "well_name" : _well_name, \
                              "field_name" : _field_name, "company" : _company, "engineer" : _engineer, \
                             "date_calc" : _date_calc}
            json_string = json.dumps(data)
            
            #data.update({"idcalc" : new_id_calc, "user_id" : _user_id, "well_name" : _well_name, \
            #                  "field_name" : _field_name, "company" : _company, "engineer" : _engineer, \
            #                 "date_calc" : _date_calc})    
            #data.update({f_id_calc : new_id_calc})
            response = requests.post(WEBHOOK_URL, json=json_string)        
            
            if response.status_code==200:
                st.success('Your data has been sent successfully')
            else:
                st.success('Then was an error sending the data')                
